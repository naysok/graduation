...
リン、カッシュの時代は、コンピュータのパワー不足、センシング等の技術の未熟さもあり、哲学や意味のついて話を進め、最後にアニメーションを見せるというスタイルである。  
渡辺、 dNA の時代になると、一定以上のマシンパワーがあり、解析であったり、現実空間から情報空間へデータを送ることだったりの操作が行われている。コンピュータを使ってこそできるようなものに近づいている。  
yang02、松川の時代になると、技術的な障壁は少なくなり、情報空間と現実空間の間でデータを介したフィードバックループや、遺伝的アルゴリズムでの形態最適化のようなことができるようになる。  
